...
void loop() {
  ...
  Serial.print(sdevice[0]);
  Serial.print(" ");
  Serial.print(sdevice[1]);
  Serial.print(" ");
  Serial.print(sdevice[2]);
  Serial.print(" ");
  Serial.print(sdevice[3]);
  Serial.print(" ");
  Serial.print(sdevice[4]);
  Serial.print(" ");
  Serial.print(sdevice[5]);
  Serial.print(" ");
  Serial.print(sdevice[6]);
  Serial.print(" ");
  Serial.print(sdevice[7]);
  Serial.print(" ");
  Serial.print(motor_st);
  Serial.println(" ");

  // ======= SEND sdevice[] AND motor_st VIA UART =======
  String uartData = "";
  for (int i = 0; i < 8; i++) {
  ...
  if(count>=15){
    ...
    if(current_cnt<1  && digitalRead(buzzer)==1){
      digitalWrite(buzzer,LOW);
      motor_st='S';
      delay(180000); //3 Min delay before esp restart
      ESP.restart();
    }
